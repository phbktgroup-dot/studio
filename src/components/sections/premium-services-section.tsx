...
const ServiceCard = ({ service }: { service: Service }) => {
  ...
  return (
    <Card className="group flex h-[420px] w-full flex-col overflow-hidden rounded-lg border bg-card shadow-md transition-all duration-300 hover:-translate-y-2 hover:shadow-xl">
      {image && (
          <Image
              src={image.imageUrl}
              alt={description}
              ...
              className="w-full object-cover aspect-[3/5] sm:aspect-[1] transition-transform duration-500 group-hover:scale-105 h-[65%] sm:h-[60%] object-center"
          />
      )}
      <div className="p-3 text-center flex-grow flex flex-col justify-center">
        <h3 className="font-bold flex items-center justify-center text-center leading-tight text-lg">{title}</h3>
        <p className="mt-1 leading-tight text-muted-foreground text-base">{description}</p>
      </div>
    </Card>
  );
};
